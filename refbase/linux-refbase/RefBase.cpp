 ...
 * =====================================================================================
 *
 *       Filename:  JrdRefBase.cpp
 *
 *    Description:  
 *
 *        Version:  1.0
 *        Created:  10/17/2011 01:22:13 PM
 *       Revision:  none
 *       Compiler:  gcc
 *
...
/*
 *shanquang wnag -
 *Mon 17 Oct 2011 01:40:22 PM CST
    ...
    void printRefs() const
    {
...

        {
            ...
            if (rc >= 0) {
                ...
                printf("STACK TRACE for %p saved in %s", this, name);
            }
            //else LOGE("FAILED TO PRINT STACK TRACE for %p in %s: %s", this,name, strerror(errno));
            else printf("FAILED TO PRINT STACK TRACE for %p in %s: %s", this,name, strerror(errno));
        }
    }
...
void RefBase::incStrong(const void* id) const
{
	...
	/* 
	 *shanquan wang 
	 * Mon 17 Oct 2011 01:41:28 PM CST
	...
	*/
    const int32_t c = __sync_fetch_and_add(&refs->mStrong,1);
    ...
    __sync_fetch_and_add(&refs->mStrong ,-INITIAL_STRONG_VALUE);
...
void RefBase::decStrong(const void* id) const
{
    ...
    const int32_t c = __sync_fetch_and_sub(&refs->mStrong,1);
    printf("decStrong of %p from %p: cnt=%d\n", this, id, c);
	/*shanuang wang
	 * Mon 17 Oct 2011 01:43:58 PM CST
...
void RefBase::forceIncStrong(const void* id) const
{
	...
	/* shanwuan wang
	 * Mon 17 Oct 2011 01:45:15 PM CST
    const int32_t c = android_atomic_inc(&refs->mStrong);
...
void RefBase::weakref_type::incWeak(const void* id)
{
    ...
    impl->addWeakRef(id);
	/* shanquan wang
	 * Mon 17 Oct 2011 01:46:58 PM CST
    const int32_t c = android_atomic_inc(&impl->mWeak);
    LOG_ASSERT(c >= 0, "incWeak called on %p after last weak ref", this);
	*/
    const int32_t c = __sync_fetch_and_sub(&impl->mWeak,1);
}
...
void RefBase::weakref_type::decWeak(const void* id)
{
	...
	/* shanquan wang 
	 * Mon 17 Oct 2011 01:49:18 PM CST
...
bool RefBase::weakref_type::attemptIncStrong(const void* id)
{
    ...
    if (curCount <= 0 || curCount == INITIAL_STRONG_VALUE) {
		...
		/*shanquan wang
		 * Mon 17 Oct 2011 02:01:25 PM CST
    ...
    if (curCount == INITIAL_STRONG_VALUE) {
		/*shanquan wang
		 * Mon 17 Oct 2011 02:02:04 PM CST
        android_atomic_add(-INITIAL_STRONG_VALUE, &impl->mStrong);
		*/
		__sync_fetch_and_add( &impl->mStrong, -INITIAL_STRONG_VALUE);
...
bool RefBase::weakref_type::attemptIncWeak(const void* id)
{
   ...
   // LOG_ASSERT(curCount >= 0, "attemptIncWeak called on %p after underflow", this);
    while (curCount > 0) {
		/* shanquan wang 
        if (android_atomic_cmpxchg(curCount, curCount+1, &impl->mWeak) == 0) {
		*/
        if (__sync_bool_compare_and_swap(&impl->mWeak,curCount, curCount+1 ) ) {
            break;
        }
        curCount = impl->mWeak;
    }
...
{
    printf("Creating refs %p with RefBase %p\n", mRefs, this);
}
...
void RefBase::extendObjectLifetime(int32_t mode)
{
    ...
    __sync_fetch_and_or(&mRefs->mFlags,mode);
}
